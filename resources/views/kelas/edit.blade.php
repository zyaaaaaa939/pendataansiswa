                ...
                <div class="p-6 text-gray-900">
                    <form action="{{ route('kelas.update', $kelas->id) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="nama_kelas" id="nama_kelas" class="shadow-sm appearance-none border border-gray-300 rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-blue-500 focus:border-blue-500" value="{{ old('nama_kelas', $kelas->nama_kelas) }}" required>
                            @error('nama_kelas')
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" name="level_kelas" id="level_kelas" class="shadow-sm appearance-none border border-gray-300 rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-blue-500 focus:border-blue-500" value="{{ old('level_kelas', $kelas->level_kelas) }}" required>
                            @error('level_kelas')
                                <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="tahun_ajar_id" id="tahun_ajar_id" class="shadow-sm appearance-none border border-gray-300 rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-blue-500 focus:border-blue-500" required>
                                ...
                                @foreach ($tahunAjars as $tahunAjar)
                                    <option value="{{ $tahunAjar->id }}" {{ old('tahun_ajar_id', $kelas->tahun_ajar_id) == $tahunAjar->id ? 'selected' : '' }}>{{ $tahunAjar->nama_tahun_ajar }}</option>
                                @endforeach
                            </select>
                            @error('tahun_ajar_id')
                                <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
