<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('jurusan.create') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                Tambah Jurusan
            </a>
        </div>
    </x-slot>
                        ...
                        <tbody>
                            @forelse ($jurusans as $jurusan)
                                <tr class="border-b hover:bg-orange-100 bg-gray-100">
                                    ...
                                    <td class="p-3 px-5">{{ $jurusan->kode_jurusan }}</td>
                                    <td class="p-3 px-5">{{ $jurusan->nama_jurusan }}</td>
                                    <td class="p-3 px-5 flex justify-end">
                                        <a href="{{ route('jurusan.edit', $jurusan->id) }}" style="background-color: #2563EB;" class="mr-2 inline-block text-sm text-white py-1 px-3 rounded focus:outline-none focus:shadow-outline transition duration-150 ease-in-out hover:bg-blue-700">Edit</a>
